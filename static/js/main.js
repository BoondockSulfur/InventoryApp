...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (genBtn && repBtn && serialIn) {
    // 1) Generieren & Zum Label-Designer weiterleiten
    genBtn.addEventListener('click', async e => {
      ...
      try {
        ...
        if (data.serial) {
          ...
          genBtn.innerText = 'Generiert!';

          // Weiterleitung zum Label-Designer mit vorausgefüllter Seriennummer
          setTimeout(() => {
            window.open(`/labels/generator?serial=${encodeURIComponent(data.serial)}`, '_blank');
          }, 500);
        } else {
